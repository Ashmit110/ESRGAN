...
def test_fn(
    ...
    for idx, (low_res, high_res) in enumerate(loop):
        ...
        with torch.cuda.amp.autocast():
            ...
            loss_critic =disc_loss(fake.detach(),real,disc)
...
def main():
    dataset = DIV2k_dataset(root_dir="D:\ESRGAN\dataset\DIV2K_valid_HR")
    loader = DataLoader(
        dataset,
    ...
    if config.LOAD_MODEL:
        load_checkpoint(
            r"training_checkpoint\train_gen_380.pth",
            gen,
            opt_gen,
            config.LEARNING_RATE,
        )
        load_checkpoint(
            r"training_checkpoint\train_disc_380.pth",
            disc,
            opt_disc,
            config.LEARNING_RATE,
        )

    print(f"about enter testing loop")
    for epoch in range(1):
        tb_step, last_low_res, last_high_res = test_fn(
            loader,
            disc,
            gen,
            opt_gen,
